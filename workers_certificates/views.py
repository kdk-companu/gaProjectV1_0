...
from workers_certificates.forms import Certificates_Workers_Form_Control, Certificates_Workers_All_Form_Control, \
    Certificates_Form_Control, Certificates_Parts_Form_Control
from workers_certificates.models import Сertificate_Users, Сertificates, Сertificate_Parts
...
class Certificate_Workers(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, ListView):
    ...
    permission_required = 'workers_certificates.certificate_users_view'
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if query_subdivision == "":
            ...
            workers = User.objects.filter(employee=query_employee).order_by('-chief__rights')
            cetificates = Сertificate_Users.objects.filter(user__employee=query_employee,
                                                           certificate__state=True).order_by('-date_delivery')
        elif department == "":
            workers = User.objects.filter(employee=query_employee, subdivision__slug=query_subdivision).order_by(
                '-chief__rights')
            cetificates = Сertificate_Users.objects.filter(user__employee=query_employee,
                                                           user__subdivision__slug=query_subdivision,
                                                           certificate__state=True).order_by('-date_delivery')
        else:
            workers = User.objects.filter(employee=query_employee, subdivision__slug=query_subdivision,
                                          department__slug=query_department).order_by('-chief__rights')
            cetificates = Сertificate_Users.objects.filter(user__employee=query_employee,
                                                           user__subdivision__slug=query_subdivision,
                                                           user__department__slug=query_department,
                                                           certificate__state=True).order_by('-date_delivery')
...
class Сertificates_View(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, ListView):
    ...
    def get(self, *args, **kwargs):
        ...
        certificates = Сertificates.objects.all()
...
class Сertificates_Update(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, UpdateView):
    ...
    login_url = 'login'
    redirect_field_name = ''
    permission_required = 'workers_certificates.certificates_change'
...
class Сertificates_Add(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, CreateView):
    ...
    login_url = 'login'
    redirect_field_name = ''
    permission_required = 'workers_certificates.certificates_add'
...
class Сertificates_Parts_Update(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, UpdateView):
    ...
    login_url = 'login'
    redirect_field_name = ''
    permission_required = 'workers_certificates.certificate_parts_change'
...
class Сertificates_Parts_Add(LoginRequiredMixin, ViewsPermissionsMixin, DataMixin, CreateView):
    ...
    login_url = 'login'
    redirect_field_name = ''
    permission_required = 'workers_certificates.certificate_parts_add'
